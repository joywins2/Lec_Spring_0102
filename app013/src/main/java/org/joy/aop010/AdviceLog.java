...
public class AdviceLog {
	...
	//...ProceedingJoinPoint : 객체는 원래 실행되어야 할 대상 메소드(핵심 기능)
	public Object profile2(ProceedingJoinPoint joinPoint) throws Throwable{
		
		//...실행되는 메서드 정보를 구함.
		String signStr = joinPoint.getSignature().toShortString();
		System.out.println(signStr + " is start2!!");
		...
		try{
			...
			long endTime = System.currentTimeMillis();
			System.out.println(signStr + " is finished2!!!");
			System.out.println(signStr + " 경과 시간2 : "+(endTime - startTime));
			System.out.println("---------------------------------------------");	
		}
	}
}
