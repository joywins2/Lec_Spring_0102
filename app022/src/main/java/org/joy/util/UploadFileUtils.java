...
public class UploadFileUtils {
	...
	/*
	 * ...549p.
	 * (DB사용이 없어)별도의 데이터가 보관될 필요가 없기 때문에 static으로 설계됨.
	 * UUID를 이용한 고유한 값 생성.
	 * ↓
	 ...
	 * ↓
	 * IF 저장한 파일이 이미지 타입(jpg, gif, png)의 파일이면, 
	 * THEN 썸네일 이미지 생성후, 썸네일 파일 이름 반환.
	 * ELSE 생성된 파일 이름 반환.
	 * 
	 */
	public static String uploadFile(String uploadPath, 
	    ...
	    if(MimeMediaUtils.getMediaType(formatName) != null){
	...
	private static void makeDir(String uploadPath, String... paths){
	    ...
	    //...https://cafe.naver.com/gugucoding/1612.
	...
	private static  String makeThumbnail(String uploadPath, //...기본경로.
	  ...
	  BufferedImage destImg = 
	      Scalr.resize(sourceImg, 
				       ...
				       //...Scalr.Mode.FIT_TO_HEIGHT설정은 썸네일 이미지 파일의 높이를
				       //뒤에 지정한 100px로 동일하게 만들어 줌.
	...
	//...557p.이미지 타입이 아닌 파일의 경우, 경로 처리를 하는 문자열의 치환용도.
